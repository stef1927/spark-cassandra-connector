...
private[connector] class KeyValueRowReader[K, V](keyReader: RowReader[K], valueReader: RowReader[V])
  ...
  override def read(row: Row, columnNames: Array[String], codecs: Array[TypeCodec[AnyRef]]): (K, V) = {
    (keyReader.read(row, columnNames, codecs), valueReader.read(row, columnNames, codecs))
  }
}
